...
// Update card ship status without refreshing entire grid
function updateCardShipStatus(orderId, isShipped) {
    const cards = document.querySelectorAll('.shipping-card');
    
    cards.forEach(card => {
        const cardOrderId = card.dataset.orderId;
        const allOrderIds = JSON.parse(card.dataset.allOrderIds || '[]');
        
        // Check if this card contains the order
        if (cardOrderId === orderId || allOrderIds.includes(orderId)) {
            // Update card class
            if (isShipped) {
            ...
            if (statusElement) {
                statusElement.className = `card-status ${isShipped ? 'status-shipped' : 'status-pending'}`;
            ...
            if (labelButton) {
                if (isShipped) {
                    labelButton.classList.add('shipped');
                    labelButton.textContent = 'SHIPPED';
                } else {
                    labelButton.classList.remove('shipped');
                    labelButton.textContent = 'LABEL';
                }
            }
            
            // Add/remove unship button
            const actionsDiv = card.querySelector('.card-actions');
            const existingUnshipButton = actionsDiv.querySelector('.unship-button');
            
            if (isShipped && !existingUnshipButton) {
                // Add unship button
                const customerName = card.querySelector('.card-customer')?.textContent || '';
                const orderNumber = card.querySelector('.card-order-number')?.textContent.replace('Order #', '') || '';
                
                const unshipButton = document.createElement('button');
                unshipButton.className = 'unship-button';
                unshipButton.textContent = 'UNSHIP';
                unshipButton.onclick = () => handleConsolidatedUnshipClick(
                    JSON.stringify(allOrderIds),
                    orderNumber,
                    customerName
                );
                actionsDiv.appendChild(unshipButton);
            } else if (!isShipped && existingUnshipButton) {
                // Remove unship button
                existingUnshipButton.remove();
            }
        }
    });
}
...
function shipCustomerOrders(customerOrders) {
    ...
    updateCardShipStatus(customerOrders[0].id, true);
    updateStatistics();
}
...
function unshipCustomerOrders(customerOrders) {
    ...
    // Update display WITHOUT full refresh
    updateCardShipStatus(customerOrders[0].id, false);
    updateStatistics();
}
